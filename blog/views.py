...
from .forms import PostForm, RecetteForm, EtapeFormset, IngredientFormset, ImageFormset, calc_dens
...
def calc_with_densimetre(request):
    if request.method == 'POST':
        ...
        return render(request, 'blog/calc_with_densimetre.html', {'dens1':str(round(dens1, 2)), 'ref_tot1':str(round(ref_tot1, 2)), 'df':df, 'di':di})

    else:
        di = "1068"
        df = "1015"
        return render(request, 'blog/calc_with_densimetre.html', {'di':di, 'df':df})